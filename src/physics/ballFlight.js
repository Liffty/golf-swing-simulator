// ballFlight.js - Updated to integrate with club model
...
// Example usage:
/*
// Direct launch with specific parameters
const ballFlight1 = new BallFlight();
const results1 = ballFlight1
  .launchBall({
    ballSpeed: 150,        // mph
    launchAngle: 15,       // degrees
    launchDirection: 0,    // degrees (0 = straight)
    spinRate: 3000,        // rpm
    spinAxis: 5            // degrees (positive = slice spin)
  })
  .simulate();

// Launch with club model
const ballFlight2 = new BallFlight();
const results2 = ballFlight2
  .launchWithClub('BLADE_7IRON', {
    clubSpeed: 85,         // mph
    attackAngle: -4,       // degrees (negative = hitting down)
    clubPath: 2,           // degrees (positive = in-to-out)
    faceAngle: 0,          // degrees (0 = square to target)
    impactLocation: {      // mm from center
      x: 5,                // positive = toward toe
      y: 2                 // positive = toward crown
    }
  })
  .simulate();

// Compare results for different club types with the same swing
const swingParams = {
  clubSpeed: 85,
  attackAngle: -4,
  clubPath: 2,
  faceAngle: 0,
  impactLocation: { x: 0, y: 0 } // Center strike
};

const bladeFlight = new BallFlight()
  .launchWithClub('BLADE_7IRON', swingParams)
  .simulate();

const cavityBackFlight = new BallFlight()
  .launchWithClub('CAVITY_BACK_7IRON', swingParams)
  .simulate();

const gameImprovementFlight = new BallFlight()
  .launchWithClub('GAME_IMPROVEMENT_7IRON', swingParams)
  .simulate();

console.log('Blade 7-iron carry:', bladeFlight.carryDistance.toFixed(1), 'yards');
console.log('Cavity Back 7-iron carry:', cavityBackFlight.carryDistance.toFixed(1), 'yards');
console.log('Game Improvement 7-iron carry:', gameImprovementFlight.carryDistance.toFixed(1), 'yards');
*/
